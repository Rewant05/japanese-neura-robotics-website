...
import { Clock3, Mail, MapPin, Newspaper, Radio, ShieldCheck, Workflow } from "lucide-react";
  ...
  return (
    <main data-interior-page className="relative min-h-screen overflow-hidden pt-24">
      ...
      <section className="container-x relative z-10 grid gap-6 pb-14 md:pb-20 lg:grid-cols-[.82fr_1.18fr]">
        <div className="interior-card tech-border relative min-h-[420px] overflow-hidden p-5 md:min-h-[500px] md:p-6">
          ...
          <div className="relative z-10 grid h-full content-between gap-5">
            <div className="max-w-md">
              <p className="hud-label text-cyan">通信ルーティング</p>
              <h2 className="mt-4 text-3xl font-semibold leading-tight text-white md:text-4xl">
                内容に合わせて、最短の担当経路へ接続します。
              </h2>
              <p className="mt-4 text-sm leading-6 text-white/62">
                研究提携、採用、メディア対応、一般相談を分類し、東京研究所の担当チームへ安全に転送します。
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {[
                ["初回確認", "24時間以内", Clock3],
                ["暗号化", "TLS保護", ShieldCheck],
                ["担当振分", "即時分類", Workflow],
              ].map(([label, value, Icon]) => (
                <div key={label as string} className="border border-white/12 bg-black/45 p-3">
                  <div className="flex items-center gap-2 text-cyan">
                    <Icon size={15} />
                    <span className="hud-label text-cyan">{label as string}</span>
                  </div>
                  <p className="mt-2 font-mono text-sm text-white">{value as string}</p>
                </div>
              ))}
            </div>
          </div>
